"""
========
run_dp.py
========
Evaluate DP stochastic gradient descent (DPSGD-LR) using lr_dp.py

Usage: python run_dp.py -d [concrete | lenses | auto]
"""

import ssl
import warnings
import sys
sys.path.append('../')

warnings.simplefilter("ignore")
ssl._create_default_https_context = ssl._create_unverified_context

import torch.nn as nn
import torch.optim as optim

from lr_dp import lrmodel, eval_dp_lr, psr_to_epsilon, DELTA
from data_loader import load_dataset, parse_datasets

PSR = 0.85  # posterior success rate (PSR); 0.85 is moderate privacy for demonstration

# per-dataset tuned hyperparameters for PSR 0.85
PARAMS = {
    "concrete": {"epochs": 30, "norm_clip": 1.5, "batch_size": 8, "lr": 1e-6},
    "lenses":   {"epochs": 30, "norm_clip": 2, "batch_size": 8, "lr": 1e-6},
    "auto":     {"epochs": 10, "norm_clip": 1, "batch_size": 16, "lr": 1e-6},
}

def run(name: str) -> None:
    """Load dataset, train DP-SGD model until convergence, and print RMSE/R2 statistics"""
    x, y, data_loader = load_dataset(name)
    params = PARAMS[name]

    epsilon = psr_to_epsilon(PSR, DELTA)  # derive ε from PSR for direct comparison with PAC-LR

    # initialize training objects (model, optimizer, loss)
    model = lrmodel(x.shape[1])
    optimizer = optim.SGD(model.parameters(), params['lr'])
    criterion = nn.MSELoss()

    print(f"Training DP-SGD for {name} dataset (PSR={PSR}, epsilon={epsilon:.4f}):")

    # train and evaluate DP-SGD model
    rmse_stats, r2_stats, _ = eval_dp_lr(
        model, optimizer, criterion, data_loader, (x, y),
        epochs=params['epochs'], epsilon=epsilon,
        norm_clip=params['norm_clip'], batch_size=params['batch_size']
    )

    # unpack stats (mean, std, median) for each metric
    rmse_mean, rmse_std, rmse_med = rmse_stats
    r2_mean, r2_std, r2_med = r2_stats

    # print results
    print(f"DP-SGD results for {name} dataset (PSR={PSR}, ε={epsilon:.4f})")
    print(f"  RMSE: mean={rmse_mean:.4f}     std. dev={rmse_std:.4f}     median={rmse_med:.4f}")
    print(f"  R2:   mean={r2_mean:.4f}    std. dev={r2_std:.4f}     median={r2_med:.4f}")

if __name__ == "__main__":
    for name in parse_datasets("DPSGD-LR"):
        run(name)
